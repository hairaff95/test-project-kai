...
<head>
    ...
    <title>Daftar Kontrak G�� KAI Tracker App</title>
...
<body class="min-h-screen bg-[#F6F7F9] dark:bg-[#282A2C] font-sans antialiased text-gray-900 dark:text-gray-100 selection:bg-blue-100 selection:text-black flex flex-col justify-between transition-colors duration-200">
                ...
                <div class="flex flex-wrap items-center gap-1.5 sm:gap-2.5 w-full sm:w-auto">
                    ...
                    <div class="relative custom-filter-container">
                        <button type="button" class="filter-dropdown-btn inline-flex items-center h-[34px] sm:h-[38px] bg-white dark:bg-[#2D3034] border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 rounded-lg lg:rounded-[10px] px-2.5 sm:px-3 py-1 transition cursor-pointer">
                            <span id="label-jenis-asset" class="{{ request('jenis_asset') ? 'text-black dark:text-white font-semibold' : 'text-gray-400 dark:text-[#9AA0A6] font-normal' }} text-xs select-none">
                    ...
                    @endphp
                    <div class="relative custom-filter-container">
                        <button type="button" class="filter-dropdown-btn inline-flex items-center gap-1 h-[34px] sm:h-[38px] bg-white dark:bg-[#2D3034] border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 rounded-lg lg:rounded-[10px] px-2.5 sm:px-3 py-1 transition cursor-pointer">
                            <span class="text-gray-400 dark:text-[#9AA0A6] font-normal text-xs select-none">Harga:</span>
                            <span id="label-harga" class="text-gray-800 dark:text-white font-semibold text-xs select-none">{{ $currentHargaLabel }}</span>
                    ...
                    <div class="relative custom-filter-container">
                        <button type="button" class="filter-dropdown-btn inline-flex items-center gap-1 h-[34px] sm:h-[38px] bg-white dark:bg-[#2D3034] border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 rounded-lg lg:rounded-[10px] px-2.5 sm:px-3 py-1 transition cursor-pointer">
                            <span class="text-gray-400 dark:text-[#9AA0A6] font-normal text-xs select-none">Waktu:</span>
                            <span id="label-waktu" class="text-gray-800 dark:text-white font-semibold text-xs select-none">{{ $currentWaktuLabel }}</span>
                            <x-icon name="chevron-down" class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-400 dark:text-[#9AA0A6] ml-0.5 pointer-events-none transition-transform duration-200 filter-dropdown-arrow" />
                        </button>
                        <div class="filter-dropdown-menu opacity-0 invisible scale-95 pointer-events-none transition-all duration-200 origin-top-left absolute left-0 top-full mt-1 z-[100] min-w-[160px] max-h-[220px] overflow-y-auto rounded-lg lg:rounded-[10px] bg-white dark:bg-[#2D3034] border border-gray-100 dark:border-white/10 shadow-[0_10px_35px_rgba(0,0,0,0.14)] dark:shadow-[0_10px_35px_rgba(0,0,0,0.6)] p-1.5 flex flex-col gap-0.5">
                            @foreach($waktuLabels as $val => $lbl)
                                <button type="button" onclick="selectFilterOption('waktu', '{{ $val }}')" class="flex items-center justify-between w-full px-2.5 py-1.5 text-xs font-semibold {{ request('waktu', '') === (string)$val ? 'bg-blue-50 dark:bg-blue-900/30 text-black dark:text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/10' }} rounded-lg lg:rounded-[10px] transition text-left cursor-pointer">
                                    <span>{{ $lbl }}</span>
                                </button>
                            @endforeach
                        </div>
    ...
    <script>
        (function () {
            ...
            // Tombol Filter G�� baru submit form
